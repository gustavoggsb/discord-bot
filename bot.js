  ...
  command(client, 'w', (message) => {
    if (message.channel.id ===  "734849330002788563"){
      if (!message.member.roles.cache.has("727637167370797187")){        
        ...
        message.guild.channels.create('Whitelist-' + userName, {
          type: 'text',
      ...
      message.channel.send(`Este comando somente pode ser realizado\nno canal #734849330002788563!`)
    }
  })
 ...
 client.on("messageReactionAdd", async (reaction, user) => {
    ...
    if (reaction.message.channel.id === "734846757665767476"){
        if (reaction.emoji.name === '✅'){
            await reaction.message.guild.members.cache.get(user.id).roles.add("734847544777113651")
        }
    }
})
